...
#   Implements a RBF with 1 input and 1 output
    ...
    def __init__(self, lr=0.001, nb_eboch=20, batch_size=1, nb_hidden=5,verbose=True):
        ...
        #np.reshape(np.array(np.linspace(0, 2*np.pi, num=self.nb_hidden)) , (1,self.nb_hidden))
        self.sigma = []
    ...
    def initialisationMu(self,X, nb_ite=200):
        ...
        for index in p:
            ...
            for index_mu in range(len(self.mu[0])):
                ...
                value = (frequency[index_mu] / count) * np.linalg.norm([current - node_mu], ord=2)
                if(value < best_value):
...
#   Implements a RBF with 1 input and 1 output
    ...
    def initialisationMu(self,X, nb_ite=10000):
        ...
        #print(self.mu)
        frequency = np.zeros(self.nb_hidden)
        self.mu = np.reshape([[10.0, 10.0],[10.0, 10.0]],(2,2))
        count = 1.0
        history = [np.copy(self.mu[:])]
        for index in p:
            #print(index)
            current = X[index]
            best_index = 0;
            ...
            for index_mu in range(len(self.mu)):
                ...
                count += 1
            frequency[best_index] += 1
            #print(self.mu[best_index])
            self.mu[best_index] = self.mu[best_index] + 0.2 * (current - self.mu[best_index])
            history.append(np.copy(self.mu[:]))
            #print(len(history))
